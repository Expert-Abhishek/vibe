...
export default function DeductionsPage() {
    const [requests, setRequests] = useState<DeductionRequest[]>([]);
    const [statusFilter, setStatusFilter] = useState<string>('Pending');
    const [searchTerm, setSearchTerm] = useState('');
    const [isLoading, setIsLoading] = useState(true);

    // Modal states
    const [selectedReq, setSelectedReq] = useState<DeductionRequest | null>(null);
    ...
    const loadRequests = () => {
        setIsLoading(true);
        fetchDeductionRequestsApi(statusFilter).then((data) => {
            setRequests(data || []);
            setIsLoading(false);
        });
    };
    ...
    const handleApprove = async (id: string) => {
        if (!confirm('Are you sure you want to approve this wallet deduction? This will deduct the amount from user wallet balance.')) return;
        setIsProcessing(true);
        const success = await approveDeductionRequestApi(id);
        setIsProcessing(false);
        if (success) {
            alert('Wallet deduction request approved and balance updated successfully!');
            setSelectedReq(null);
            loadRequests();
        } else {
            alert('Failed to approve deduction request.');
        }
    };
    ...
    const handleReject = async (id: string) => {
        ...
        setIsProcessing(true);
        const success = await rejectDeductionRequestApi(id, rejectReason.trim());
        setIsProcessing(false);
        if (success) {
            alert('Deduction request rejected.');
            setSelectedReq(null);
            setRejectReason('');
            setShowRejectForm(false);
            loadRequests();
        } else {
            alert('Failed to reject deduction request.');
        }
    };
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-white tracking-tight flex items-center gap-2">
                        <ArrowDownRight className="w-6 h-6 text-brand-500" />
                        <span>Wallet Deduction Approvals</span>
                    </h1>
                    <p className="text-xs text-dark-textMuted mt-1">
                        Review and approve customer wallet deduction/payment requests. Approving a request will deduct the amount from their wallet balance.
                    </p>
                </div>
                ...
                <div className="flex items-center space-x-3">
                    ...
                    >
                        ...
                        <option value="Approved">Approved Deductions</option>
                        <option value="Rejected">Rejected Deductions</option>
                    </select>
            ...
            <div className="glass-card rounded-2xl overflow-hidden border border-dark-border shadow-xl">
                {isLoading ? (
                    <div className="py-16 flex flex-col items-center justify-center space-y-4">
                        ...
                        <p className="text-xs text-dark-textMuted font-bold">Fetching deduction ledger...</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left border-collapse text-xs">
                            <thead>
                                <tr className="bg-dark-hover/80 text-dark-textMuted font-bold border-b border-dark-border uppercase tracking-wider text-[11px]">
                                    ...
                                    <th className="py-4 px-6">Deduction Amount</th>
                            ...
                            <tbody className="divide-y divide-dark-border/60">
                                {filteredRequests.map((req) => (
                                    <tr key={req.id} className="hover:bg-dark-hover/40 transition-colors">
                                        ...
                                        <td className="py-4 px-6 font-bold text-red-500 text-sm">
                                            ₹{parseFloat(req.amount.toString()).toLocaleString('en-IN')}
                                        </td>

                                        <td className="py-4 px-6 text-gray-300 max-w-xs truncate">
                                            {req.description || <span className="italic text-dark-textMuted text-[11px]">No reason provided</span>}
                                        </td>
                                ...
                                {filteredRequests.length === 0 && (
                                    <tr>
                                        <td colSpan={statusFilter === 'Rejected' ? 9 : 8} className="py-12 text-center text-dark-textMuted">
                                            No deduction request records found.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-dark-card border border-dark-border rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
                        ...
                        <div className="p-6 border-b border-dark-border flex items-center justify-between">
                            <div>
                                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Deduction Request Details</h2>
                                <p className="text-[10px] text-dark-textMuted mt-0.5">
                                    Verify the wallet payment request of {selectedReq.user_name} (₹{parseFloat(selectedReq.amount.toString()).toLocaleString('en-IN')})
                                </p>
                            </div>
                            <button
                                onClick={() => {
                        ...
                        <div className="p-6 space-y-4 flex-1">
                            ...
                            <div className="w-full h-64 rounded-xl overflow-hidden border border-dark-border bg-dark-bg flex items-center justify-center">
                                {selectedReq.screenshot_url ? (
                                    <img
                                        src={selectedReq.screenshot_url}
                                        alt="Payment deduction screenshot proof"
                                        className="w-full h-full object-contain"
                                    />
                                ) : (
                                    <div className="flex flex-col items-center justify-center text-dark-textMuted">
                                        <AlertCircle className="w-8 h-8 mb-2" />
                                        <span className="text-xs font-semibold">No screenshot attachment provided</span>
                                        <span className="text-[10px] text-dark-textMuted mt-0.5">Deduction is based on description & amount</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="p-4 bg-dark-bg rounded-xl border border-dark-border space-y-2 text-xs text-gray-300">
                                <div>
                                    <span className="text-dark-textMuted font-bold block mb-0.5">User Details:</span>
                                    <span className="font-bold text-white">{selectedReq.user_name}</span> ({selectedReq.role})
                                    <span className="block text-[10px] text-dark-textMuted font-mono mt-0.5">ID: {selectedReq.user_id}</span>
                                </div>
                                <div>
                                    <span className="text-dark-textMuted font-bold block mb-0.5">Amount to Deduct:</span>
                                    <span className="font-extrabold text-red-500 text-sm">₹{selectedReq.amount}</span>
                                </div>
                                <div>
                                    <span className="text-dark-textMuted font-bold block mb-0.5">Description / Reason:</span>
                                    <span className="italic text-white">"{selectedReq.description || 'No explanation provided'}"</span>
                                </div>
                            </div>
                            ...
                            {showRejectForm && (
                                <div className="space-y-2">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="e.g. Balance mismatch or transaction not recognized"
                                        value={rejectReason}
                                ...
                                <div className="flex items-center space-x-2">
                                    {showRejectForm ? (
                                        ...
                                        <>
                                            ...
                                            >
                                                {isProcessing ? 'Approving...' : 'Approve & Deduct'}
                                            </button>
                                        </>
                                    )}
                                </div>
