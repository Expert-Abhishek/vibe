...
export default function AdminTripsPage() {
  ...
  const filteredTrips = trips.filter((t) => {
    ...
    if (activeFilter === 'active') {
      matchesFilter = statusLower.includes('active') || statusLower.includes('accepted') || statusLower.includes('start') || statusLower.includes('arrived');
    } else if (activeFilter === 'upcoming') {
      matchesFilter = statusLower.includes('pending') || statusLower.includes('confirm') || t.bookingType === 'PRE_BOOKED';
  ...
  const getBadgeClass = (statusStr: string) => {
    ...
    if (s.includes('active') || s.includes('start') || s.includes('arrived')) return 'bg-blue-500/10 text-blue-400 border-blue-500/20';
    if (s.includes('accepted') || s.includes('confirm')) return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20';
    if (s.includes('complete')) return 'bg-brand-500/10 text-brand-400 border-brand-500/20';
    if (s.includes('cancel') || s.includes('decline')) return 'bg-rose-500/10 text-rose-400 border-rose-500/20';
    return 'bg-amber-500/10 text-amber-400 border-amber-500/20';
  };

  return (
    <div className="flex min-h-screen bg-dark-bg text-dark-textPrimary">
      <Sidebar />
                      ...
                      <tr key={t.id} className="hover:bg-dark-hover/40 transition-colors">
                        ...
                        <td className="p-4">
                          <div className="font-semibold text-emerald-400 flex items-center space-x-1.5">
                            ...
                            <span>{t.driverOrGuideName || 'Assigned Captain'}</span>
                          </div>
