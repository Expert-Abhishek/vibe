...
import { deleteUserApi, fetchDriversApi, updateDriverRateApi, updateUserStatusApi, adjustWalletBalanceApi } from '@/lib/api';
      ...
      {selectedDriver && (
        <DriverDetailModal
          ...
          onAdjustBalance={async (amount, reason) => {
            const success = await adjustWalletBalanceApi(selectedDriver.id, amount, reason);
            if (success) {
              setDriversList((prev) =>
                prev.map((d) => (d.id === selectedDriver.id ? { ...d, walletBalance: d.walletBalance + amount } : d))
              );
              setSelectedDriver((prev) => (prev ? { ...prev, walletBalance: prev.walletBalance + amount } : null));
            }
            return success;
          }}
        />
      ...
      <div className="bg-dark-card border border-dark-border rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          <div className="p-5 rounded-2xl bg-dark-hover/60 border border-dark-border/80 space-y-3">
            ...
            <p className="text-[11px] text-dark-textMuted">
              Manually add money or deduct money from this driver's wallet.
            </p>
